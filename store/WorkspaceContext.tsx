...
import { useChatbot, useGuardrails, useKnowledgeBase } from '@/hooks';
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
    ...
    const guardrails = useGuardrails(currentWorkspace?.id);
    const chatbots = useChatbot(currentWorkspace?.id);
    const knowledgeBase = useKnowledgeBase(currentWorkspace?.id);

    return (
        <WorkspaceContext.Provider value={{ currentWorkspace, setCurrentWorkspace, ...chatbots, ...guardrails, ...knowledgeBase }}>
            {children}
